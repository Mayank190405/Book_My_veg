 ...
 */
export function scanForThreats(buffer: Buffer): { safe: boolean; reason?: string } {
 ...
 */
export const scanImageUpload = (req: Request, res: Response, next: NextFunction) => {
    upload.single("image")(req, res, async (err: any) => {
        ...
        if (!validateMagicBytes(file.buffer, ext)) {
            logger.warn(`[Security Alert] Magic bytes mismatch for file ${file.originalname} (claimed ${ext})`);
            return res.status(400).json({ 
        ...
        if (!scanResult.safe) {
            ...
            logger.error(`[SECURITY THREAT] Malicious image upload blocked from ${clientIp}. Reason: ${reason}`);
            ...
            try {
                ...
                logger.error("Error logging security incident for malicious file upload:", loggingError);
            }
