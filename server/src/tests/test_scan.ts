import { validateMagicBytes, scanForThreats } from "../middleware/imageScan";

interface TestCase {
    name: string;
    buffer: Buffer;
    ext: string;
    expectedMagic: boolean;
...
function runSuite() {
    ...
    for (const tc of testCases) {
        ...
        const magicOk = validateMagicBytes(tc.buffer, tc.ext);
        const scanOk = scanForThreats(tc.buffer).safe;
